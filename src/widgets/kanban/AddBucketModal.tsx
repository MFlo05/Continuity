...
// to inherit its existing portal token-bridge for free — see
// DESIGN_SYSTEM.md gotcha #4. No .cc2-root prefix anywhere here.
export function AddBucketModal({ existingBucketNames, error, onCancel, onConfirm }: Props) {
  ...
  const duplicate    = existingBucketNames.some(n => n.toLowerCase() === trimmed.toLowerCase());
  const invalidChars = /[#[\]\n]/.test(trimmed);
    ...
    <div className="cc2-modal-backdrop" onMouseDown={onCancel}>
      <div className="cc2-modal cc2-setup-modal" onMouseDown={e => e.stopPropagation()}>
        ...
        <div className="cc2-modal-header">
          <span className="cc2-modal-title">New Bucket</span>
          <button className="cc2-modal-close" onClick={onCancel}>✕</button>
        </div>

        <div className="cc2-setup-body">
          <p className="cc2-setup-hint">
            Name a new Kanban bucket. It's added as a <strong>## header</strong> in the TODO list.
          </p>
          ...
          />
          {duplicate && <div className="cc2-kb-bucket-error">A bucket with that name already exists.</div>}
